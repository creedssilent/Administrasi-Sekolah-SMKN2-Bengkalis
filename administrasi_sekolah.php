...
// Fetch school administration data
$administrasi_sekolah_query = $conn->query("SELECT * FROM administrations");
if (!$administrasi_sekolah_query) {
    die("Error: " . $conn->error);
}

// Add new school administration record
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['title'], $_POST['description'], $_POST['amount'])) {
    $title = $_POST['title'];
    $description = $_POST['description'];
    $amount = $_POST['amount'];
    $query = "INSERT INTO administrations (title, description, amount, created_at) VALUES ('$title', '$description', '$amount', NOW())";
    if ($conn->query($query)) {
        echo "<script>alert('Administrasi sekolah berhasil ditambahkan!'); window.location.href='administrasi_sekolah.php';</script>";
    } else {
        echo "<script>alert('Gagal menambahkan administrasi sekolah: " . $conn->error . "');</script>";
    }
}
?>
...
<body>
    ...
    <!-- Container Utama -->
    <div class="container">
        <h2 class="dashboard-title">ADMINISTRASI SEKOLAH</h2>

        <div class="card">
            <h3>Tambah Administrasi Sekolah</h3>
        ...
        <div class="card">
            <h3>Data Administrasi Sekolah</h3>
            <table>
                <thead>
                    <tr>
                        <th>Judul</th>
                ...
                <tbody>
                    <?php while ($row = $administrasi_sekolah_query->fetch_assoc()) : ?>
                        <tr>
