...
$kegiatan_query = $conn->query("SELECT * FROM activities");
if (!$kegiatan_query) {
    die("Error: " . $conn->error);
}

// Tambah kegiatan
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nama_kegiatan'], $_POST['deskripsi_kegiatan'], $_POST['tanggal_kegiatan'])) {
    $nama_kegiatan = $_POST['nama_kegiatan'];
    $deskripsi_kegiatan = $_POST['deskripsi_kegiatan'];
    $tanggal_kegiatan = $_POST['tanggal_kegiatan'];
    $query = "INSERT INTO activities (title, description, amount) VALUES ('$nama_kegiatan', '$deskripsi_kegiatan', '$tanggal_kegiatan')";
    if ($conn->query($query)) {
        echo "<script>alert('Kegiatan berhasil ditambahkan!'); window.location.href='administrasi_kegiatan.php';</script>";
    } else {
        echo "<script>alert('Gagal menambahkan kegiatan: " . $conn->error . "');</script>";
    }
}
?>
...
<head>
    ...
    <title>Administrasi Kegiatan</title>
...
<body>
    ...
    <!-- Container Utama -->
    <div class="container">
        <h2 class="dashboard-title">ADMINISTRASI KEGIATAN</h2>

        <div class="card">
            <h3>Tambah Kegiatan</h3>
            <form method="POST" class="form-container">
                <input type="text" name="nama_kegiatan" placeholder="Nama Kegiatan" required>
                <textarea name="deskripsi_kegiatan" placeholder="Deskripsi Kegiatan" rows="4" required></textarea>
                <input type="date" name="tanggal_kegiatan" required>
                <button type="submit"><i class="fas fa-plus"></i> Tambah Kegiatan</button>
            </form>
        </div>
        ...
        <div class="card">
            ...
            <table>
                ...
                <tbody>
                    <?php while ($row = $kegiatan_query->fetch_assoc()) : ?>
                        <tr>
